...
- **Fetch-failure policy (refined 2026-07-13): keep a still-valid last-good reading; show "Weather error" only when nothing valid for today.** Weather is a *daily* forecast (today's high/low/condition), so a last-good reading that's still for **today's local date** is at worst a few hours old and perfectly usable — better than an error (owner's call). `weather.parse_weather` now carries the forecast's own `date` (Open-Meteo `daily.time[0]`); on a failed/unusable pull `display_loop` keeps `last_weather` (clears `weather_error`) when `last_weather["date"] == _local_today_iso()`, and only sets the explicit **"Weather error"** (`display.WEATHER_ERROR`) when there's no last-good at all or it's from a **prior day** (e.g. across midnight during an outage) — genuinely stale. The date test subsumes both "wrong date" and "too old". This replaces the earlier always-error-on-failure rule (which flashed "Weather error" over one transient bad payload even with a perfectly good reading in hand). The **retry-every-render-tick** behavior now fires **only while `weather_error` is actually set** (nothing valid to show → clear it fast); while showing a valid last-good reading there's no urgency, so it just waits out the normal (up to 30 min) bucket. The honest-error reasoning still holds for the genuinely-stale case — same as the per-stop STALE badge, don't show prior-day data next to a live clock unmarked.

...
- **The 48 KB framebuffer is allocated ONCE per boot/wake and reused for the whole cycle** (`main()` immediately after validated settings/config and deep-sleep mode selection in deployed mode; `display_loop` in the legacy awake mode). In deep-sleep mode this reservation must happen **before RTC decode, Wi-Fi connect, cold-boot NTP, or any network work**: hardware testing on 2026-07-25 found ~91 KB total free after those activities but no contiguous 40 KB block, so allocating inside `deep_sleep_cycle` raised `MemoryError`. The reserved `fb`/`fb_buf` are passed into the cycle. It is deliberately lost during ESP32 deep sleep and reconstructed from compact semantic RTC state when needed. This reverses an earlier transient-per-refresh rule — see "RAM-vs-HTTPS conflict (RESOLVED)". SL + Open-Meteo now use plain **HTTP**, so the resident buffer no longer starves a TLS handshake. Allocating once on the clean boot heap (~90 KB contiguous) also avoids later `MemoryError` from fragmentation; MicroPython's GC never compacts.
...
- **RTC state compatibility is strict** (retained schema v2, renderer revision 1; settled 2026-07-25). Restore validates every nested display/status/weather field plus integer timestamps, then requires both a deterministic fingerprint of render-affecting normalized settings and the exact ordered `site_id:direction` section identities. Old v1 data, changed settings, changed renderer semantics, or reordered stops safely behave like a cold wake and force a full refresh. The checksum detects corruption; it is not authentication.
- **A changed frame is an RTC/panel transaction** (settled 2026-07-25): encode the proposed state first, invalidate RTC memory and verify it is empty, refresh the panel, then commit exact encoded bytes and validate their readback against the current settings/stop context. Encoding or invalidation failure must happen before the panel is touched; even `EPD7in5V2()` construction (which configures pins/SPI) is deferred into the refresh callback after invalidation. Refresh failure leaves RTC empty. Any commit write/readback/decode exception best-effort clears and verifies RTC before the original error is re-raised; cleanup failure is reported without masking that original error. A `none` decision skips invalidation and all panel construction/access, but may verified-commit timestamp-only state whose semantic frame is identical.
...
- **Every adapter call makes one network attempt** (settled 2026-07-25): deep sleep's next minute wake and the awake loop's next scheduled pull are the retries. Feed the WDT between sources; this prevents one unavailable slow-timeout source (observed with Open-Meteo) from consuming the whole 150 s watchdog budget and blocking departures/display/RTC-state persistence.
...
- **Shared single-attempt timeout policy** (`sl.py` + `openmeteo.py`, settled 2026-07-25): `timeout_s=10` bounds one request. Do not add adapter retries or retry delays: the next scheduled pull is the retry, preserving time for the other sources and the display/RTC persistence. Keep the two modules' timeout defaults in sync (see main.py's `WDT_TIMEOUT_MS` comment for the per-tick time budget).

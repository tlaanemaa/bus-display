...
- **The 48 KB framebuffer is allocated ONCE per boot/wake and reused for the whole cycle** (deep-sleep `main()` allocates it before RTC/network/panel work and passes it to `deep_sleep_cycle`; legacy awake mode allocates it in `display_loop`). It is deliberately lost during ESP32 deep sleep and reconstructed from compact semantic RTC state when needed. This reverses an earlier transient-per-refresh rule — see "RAM-vs-HTTPS conflict (RESOLVED)". SL + Open-Meteo now use plain **HTTP**, so the resident buffer no longer starves a TLS handshake. Allocating once on the clean boot heap (~90 KB contiguous) also avoids later `MemoryError` from fragmentation; MicroPython's GC never compacts.
...
- The one framebuffer and wake watchdog are created before RTC, Wi-Fi, NTP, or panel work. Each wake resets the STA interface before connecting, and the watchdog is fed through Wi-Fi polling, boundary preparation, requests, and NTP.
...
- **The WDT (`machine.WDT(150 s)`) stays** as the general hang backstop — force-reboots on any hang, Python-catchable or not. Deep-sleep wakes own one from boot and feed it through Wi-Fi, preparation, requests, and NTP; awake mode feeds it each `display_loop` iteration. It should now rarely fire (no TLS to hang); if it starts firing every boot, that's a regression — check for a new eager unconditional import or a resident allocation that broke the boot-time framebuffer alloc.

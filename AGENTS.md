...
- **A changed frame is an RTC/panel transaction** (settled 2026-07-25): encode the proposed state first, invalidate RTC memory and verify it is empty, refresh the panel, then commit exact encoded bytes and validate their readback against the current settings/stop context. Encoding or invalidation failure must happen before the panel is touched; refresh failure leaves RTC empty; commit/readback failure clears the untrusted bytes so the next wake cannot differential-refresh against stale state. A `none` decision skips invalidation and the panel, but may verified-commit timestamp-only state whose semantic frame is identical.

...
- **The 48 KB framebuffer is allocated ONCE per boot/wake and reused for the whole cycle** (deep-sleep `app.main()` constructs the WDT, lets `wifi.connect_sta(..., wdt=wdt)` reserve the Wi-Fi driver's RX buffers, then allocates the framebuffer before RTC/NTP/API/panel work and passes it to `deep_sleep_cycle`; legacy awake mode allocates it in `display_loop`). This order is hardware-confirmed 2026-08-06: allocating the framebuffer before STA initialization left only 4 of 10 expected RX buffers, logged deinit failure `0x3001`, and raised `OSError: WiFi Out of Memory`. The reverse order with the 41.9 KB runtime still in source `main.py` connected Wi-Fi, then failed allocating 48,000 bytes because on-device compilation had already fragmented the remaining heap. The runtime therefore lives in host-precompiled `app.mpy`; tiny source `main.py` only imports it. The buffer is deliberately lost during ESP32 deep sleep and reconstructed from compact semantic RTC state when needed. This reverses an earlier transient-per-refresh rule — see "RAM-vs-HTTPS conflict (RESOLVED)". SL + Open-Meteo now use plain **HTTP**, so the resident buffer no longer starves a TLS handshake. Allocating once per wake after STA initialization also avoids later `MemoryError` from fragmentation; MicroPython's GC never compacts.
...
```
deploy.bat                                         # compile app/runtime, copy support, activate tiny main.py last, reset
python -m mpy_cross src/app.py                     # runtime-only quick build (deploy.bat is safer for normal use)
mpremote connect COM3 fs cp src/app.mpy :app.mpy   # runtime-only quick copy; then reset
mpremote connect COM3 fs cp src/main.py :main.py   # boot shim only -- almost never changes
mpremote connect COM3 reset                        # restart so new code runs (cp does NOT restart)
mpremote connect COM3 repl                         # serial console; Ctrl-] exits, Ctrl-C interrupts app runtime
...
**The whole runtime ships as precompiled `.mpy`, not source** (`deploy.bat` runs `mpy-cross` on every `.py` except `main.py`, then copies only the bytecode). This keeps the boot heap clean and unfragmented on this PSRAM-less board: on-device compilation spikes/fragments RAM (parser/AST), which threatens the resident 48 KB framebuffer's one-time allocation (and historically starved the TLS fetch — see "RAM-vs-HTTPS conflict (RESOLVED)"). The 41.9 KB runtime is `app.py` on the host and `app.mpy` on-device. `main.py` is the only source exception because MicroPython auto-runs that filename; it is deliberately tiny and only imports `app`. `deploy.bat` copies `app.mpy`, other modules, JSON, vendored bytecode, and fonts first, then copies source `main.py` exactly once immediately before reset; every copy is error-checked, so the shim is not activated until its runtime and support files succeed. For any runtime module, recompile first (`python -m mpy_cross src/foo.py` then `mpremote connect COM3 fs cp src/foo.mpy :foo.mpy`) or use `deploy.bat`. The `.mpy` files are **gitignored build artifacts** — `.py` is the source of truth. Requires `pip install mpy-cross`.

`deploy.bat` does not compile `src/main.py` and explicitly skips any ignored `src/main.mpy`; only source `main.py` is the boot entry point. `app.py` is included by the ordinary wildcard and deployed as `app.mpy` before that activation copy.

Only one process can hold the COM port — close any open REPL before deploying. `main.py` auto-runs on boot and imports `app.mpy`: unexpected deep-sleep runtime failures use the bounded recovery path, while configuration errors and the awake fallback stay Ctrl-C/REPL-recoverable. Never create a tight reset loop, or the board becomes hard to reflash.
...
Boot loads `/settings.json` and `/config.json`, constructs the watchdog, initializes Wi-Fi STA, allocates the one framebuffer, restores validated RTC state, waits for the next minute boundary, fetches, renders only changed content, saves retained state, and sleeps until the next prepared wake. The two JSON boundaries are deliberately strict: settings and the optional nested Wi-Fi object must have safe types/ranges, while unknown legacy settings keys remain compatible; invalid settings/configuration surfaces as a REPL-recoverable error rather than entering automatic deep-sleep recovery. Stop names must contain non-whitespace text and are capped at `settings.MAX_STOP_NAME_CHARS` (48), which preserves the committed example's 42-character labels while keeping the maximum tested two-stop semantic frame within the 2048-byte RTC budget. This owner-managed device is configured over USB; it never falls into a permanent setup AP after a transient router outage. **Missing credentials or unavailable Wi-Fi**: draw a clear on-screen Wi-Fi status so a wall-powered, unattended display never remains silently stale, then retry on the next wake. Wi-Fi credentials still live only on the device. A better onboarding experience may be designed later if the project expands.

**NTP is re-synced periodically, not only at cold boot**: hardware measurement on 2026-08-06 found the deep-sleep RTC had gained 16 seconds in roughly an hour, so both deep-sleep and legacy awake modes use a five-minute resync interval (`app._NTP_RESYNC_INTERVAL_S`). Deep-sleep mode carries a successful cold-boot NTP epoch into the first cycle/state (so that same sync is not immediately repeated), retains the last successful epoch in RTC state, and performs a due sync before waiting for the request boundary. That ordering corrects RTC drift before deciding when `HH:MM:00` occurs and ensures weather date/usability checks and footer construction see the corrected clock together. Failures do not advance the successful-sync marker, so the next wake retries. Departure countdowns are server-computed by SL and unaffected by RTC drift; the footer clock/date and tick alignment depend on NTP.

**Deep-sleep reliability transaction (settled 2026-08-06)**:

- Retained state is schema v2 with renderer and settings compatibility markers plus strict nested frame validation. Empty, corrupt, oversized, old, or incompatible bytes are rejected and force a full refresh.
- The wake watchdog is created first and passed into Wi-Fi initialization; after STA has reserved its RX buffers, the one framebuffer is allocated before RTC, NTP, API, render, or panel work. Each wake resets the STA interface before connecting, and the same watchdog is fed through Wi-Fi polling, boundary preparation, requests, and NTP.
- A changed frame is ordered exactly: encode -> strict-decode the candidate in the current settings/section context -> RTC invalidate/readback -> panel refresh/sleep -> exact-byte RTC commit/readback. An invalid candidate touches neither RTC nor EPD. An unchanged frame commits retained state without constructing the EPD. Fresh display rows are canonical JSON-style lists, so tuple/list representation differences cannot cause a spurious refresh. Rendering the reconstructed old partial plane is deliberately not logged as a visible redraw.
- Future retained timestamps are treated as due, never as fresh. A weather reading is revalidated on every wake, and a prior-day forecast forces an immediate pull even inside the normal weather interval. Unexpected deep-sleep runtime failures best-effort disable STA, invalidate retained state, and sleep for 60 seconds before retrying. Both ordinary and recovery `machine.deepsleep(...)` calls are followed by `machine.reset()` as a hard fallback if the sleep contract is violated and the call returns. `KeyboardInterrupt`, invalid configuration, and the awake fallback remain USB/REPL-recoverable.

```
src/                 # maps 1:1 to the device filesystem root
  main.py            # tiny source boot shim: import app (activation copied last)
  app.py             # full runtime/boot flow; deployed precompiled as app.mpy
...
- Today's forecast: `GET http://api.open-meteo.com/v1/forecast?latitude=..&longitude=..&daily=temperature_2m_max,temperature_2m_min&hourly=weather_code,precipitation_probability&timezone=auto&forecast_days=1` — **no API key**, like SL. `openmeteo.py` fetches, `weather.py` parses (pure).
...
- **Open-Meteo, not SMHI, on purpose**: SMHI's point forecast returns the whole multi-day hourly series (100 KB+); Open-Meteo lets us request **only today's daily + hourly fields → a couple KB**. `timezone=auto` so the daily min/max and hourly timestamps aggregate over the LOCAL day.
...
- **Condition + precip are a daytime MODE/MAX over hourly data, not Open-Meteo's own daily aggregate (fixed 2026-07-13).** Diagnosed after the glyph showed "cloudy" on a fully clear day: Open-Meteo's `daily.weather_code`/`precipitation_probability_max` are each the **MAX (most severe) value across the full 24 h day**, so a single overcast or drizzly hour at 3 AM held the whole day's glyph hostage even though nobody's awake to experience it. Fix: request `hourly=weather_code,precipitation_probability` instead, and in `weather.py` (pure, tested) restrict to a **07:00–23:00 local daytime window** (`DAYTIME_START_HOUR`/`DAYTIME_END_HOUR`, read off the hourly block's local timestamps — `timezone=auto` makes this a fixed string slice, no date math), then:
  - **condition** = `dominant_condition()` — the most common **bucket** (not raw WMO code, so e.g. slight vs moderate rain don't split the vote) among daytime hours. Ties (possible with up to 16 samples) break toward the more notable/severe bucket (`_SEVERITY` ranking, clear < … < thunder) — consistent with the app surfacing rather than hiding uncertain info (STALE badges, explicit "Weather error").
  - **precip** = max daytime probability (same daytime-only reasoning as condition — overnight rain doesn't affect the umbrella decision either).
  - **tmax/tmin stay on the `daily` block**, deliberately not daytime-restricted — the overnight low is still relevant for dressing.
...
- **Fetch-failure policy (refined 2026-07-13): keep a still-valid last-good reading; show "Weather error" only when nothing usable.** Weather is a *daily* forecast, so a recent last-good reading is more useful than an error during a fetch outage (owner's call). BUT it isn't static — **Open-Meteo revises the daily forecast through the day** as new model runs arrive (verified against the docs: DWD ICON re-runs ~every 3 h, GFS/Met-France hourly; precip probability especially can swing). So "keep last-good" is bounded on **two** axes, both in the pure `weather.keep_last_good(reading, today_iso, age_s, max_age_s)`:
  ...
  2. **Fresh enough** — fetched no more than `max_age_s` ago (`weather.max_age_min`, **default 180 = 3 h**, ~one ICON cycle; configurable in the weather block). Bounds how stale the *outage fallback* can get, since the forecast revises during the day. `app.py` tracks `last_weather_time` (`time.time()` of the last successful pull) and passes `age_s = now - last_weather_time`.
...
- **Config in `/settings.json`, gitignored** (stop ids, direction, forecast, cadence) so the owner's home stop isn't in a public repo. `settings.py` loads it (mirrors `config.py`). `src/settings.example.json` is the committed template (uses the public Slussen siteId as a placeholder). Edit + deploy it like any file — no `app.py` change for settings-only edits. JSON not YAML (MicroPython has `json` built in, no YAML parser).
- **Stops**: a flat, ordered list of `{name, site_id}`; every stop is always fetched and shown in order (no primary/fallback/suitability logic). Names render on-screen as section labels, must not be blank/whitespace-only, and are limited to 48 characters (`settings.MAX_STOP_NAME_CHARS`). The bound keeps the committed 42-character example valid; the worst-case regression with two 48-character non-BMP names, three departures per stop, weather/status, and maximum test fields encodes to 1975/2048 bytes (73-byte headroom).
...
- **Fetch strategy**: each stop is fetched independently every cycle (`app.py:_fetch_all_stops`); one stop failing keeps its last-good departures and flags **that stop** stale (a per-stop STALE badge, see "Screen design"), without touching the others. `stale_flags = [r is None for r in results]` — tied to whether the fetch **errored**, not to the data shape. If *every* stop errors and none has prior data, the screen still renders ("No departures" + STALE everywhere) rather than freezing silently.
...
- **Refresh cadence — three intervals, in MINUTES** (`app.py` converts to seconds internally):
...
- **Deep sleep is the deployed operating mode** (hardware-verified 2026-07-25; enabled by `power.deep_sleep: true`). Each wake reconstructs the previous semantic frame from validated ESP32 RTC user memory, renders it into the single 48 KB framebuffer for partial plane `0x10`, then reuses that buffer for the new `0x13` frame. The framebuffer itself is never retained or written to flash. See “Deep-sleep reliability transaction” for the v2 validation and commit ordering.
- **One-minute wall-clock cadence is a fixed product requirement** (settled 2026-07-19): departures' relative times and the footer clock both lose substantial utility at a slower cadence. Deep sleep wakes `wake_advance_s` (default **5**, raised from 3 after normal hardware preparation measured 1.6–2.4 seconds) before the next minute, corrects due RTC drift from NTP, then holds requests until the `HH:MM:00` boundary. The intended request boundary is captured before variable-duration Wi-Fi/framebuffer preparation; otherwise an occasional association finishing just after `:00` silently targets the following minute and skips a displayed minute (fixed 2026-08-06). The advance is a tuning knob for measured boot/Wi-Fi preparation time; zero retains boundary-wake behavior.
...
- **The WDT (`machine.WDT(150 s)`) stays** as the general hang backstop — force-reboots on any hang, Python-catchable or not. Deep-sleep wakes own one before Wi-Fi initialization and feed it through Wi-Fi, preparation, requests, and NTP; awake mode feeds it each `display_loop` iteration. It should now rarely fire (no TLS to hang); if it starts firing every boot, that's a regression — check for a new eager unconditional import or a resident allocation that broke the Wi-Fi-then-framebuffer boot order.
...
- **Shared retry/timeout policy** (`sl.py` + `openmeteo.py`, settled 2026-07-12): `retries=3`, `timeout_s=10` (aggressive on purpose — a request that hasn't returned in 10s is treated as dead and retried, not waited out), `RETRY_DELAY_S=3` between attempts (not after the last one). Every response must be 2xx and have its expected top-level JSON shape, and is closed in `finally` on both success and failure. Worst case for one fetch: `3*10 + 2*3 = 36s`. Keep the two modules' defaults in sync (see `app.py`'s `WDT_TIMEOUT_MS` comment for why the numbers matter — they're the whole per-tick time budget).
...
- **The whole runtime ships as precompiled `.mpy` (`deploy.bat` compiles every `.py` but tiny `main.py`), because on-device compilation spikes/fragments the heap.** First seen vendoring `microdot.py` (58 KB): compiling it *on device* threw `OSError: WiFi Out of Memory`. Confirmed again 2026-08-06 when source `main.py` (41.9 KB) let Wi-Fi connect but then `bytearray(48000)` failed. The fragmentation metric is **largest contiguous free block**, not `gc.mem_free()` (`micropython.mem_info()` → "max new split" / "max free sz"): compiling on-device once collapsed contiguous free to ~7 KB. Precompiling runtime `app.py` keeps the boot heap clean for both Wi-Fi and the resident framebuffer. (Historically this also unblocked the TLS fetch, now moot with HTTP — see "RAM-vs-HTTPS conflict (RESOLVED)".) So: compile runtime code on the host, never the device. PyPI's `mpy-cross` (1.27.0) lags the firmware (1.28.0) but the bytecode matches as of 2026-07 — if a future `.mpy` fails to import with a version error, check this first.

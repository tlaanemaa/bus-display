...
**NTP is re-synced periodically, not only at cold boot**: deep-sleep mode carries a successful cold-boot NTP epoch into the first cycle/state (so that same sync is not immediately repeated), retains the last successful epoch in RTC state, and retries at most daily after the request boundary. A due resync runs after departures but before weather date/usability checks and footer construction, so an NTP correction across local midnight cannot pair today's footer with yesterday's forecast. The legacy awake loop uses a daily wall-clock bucket. Failures do not advance the successful-sync marker, so a later cycle retries. Departure countdowns are server-computed by SL and unaffected by RTC drift; only the footer clock/date and tick alignment depend on it.

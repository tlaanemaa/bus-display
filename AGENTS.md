...
- A changed frame is ordered exactly: encode → RTC invalidate/readback → panel refresh/sleep → exact-byte RTC commit/readback. An unchanged frame commits retained state without constructing the EPD. Rendering the reconstructed old partial plane is deliberately not logged as a visible redraw.
- Future retained timestamps are treated as due, never as fresh. Unexpected deep-sleep runtime failures invalidate retained state and sleep for 60 seconds before retrying; `KeyboardInterrupt`, invalid configuration, and the awake fallback remain USB/REPL-recoverable.
